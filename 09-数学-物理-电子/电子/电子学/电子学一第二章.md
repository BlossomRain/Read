...
> ◎ 结构：**2.3.1** 理想积分器（$R$–$C$ 反相）→ **2.3.2** 方波→三角波 → **2.3.3–2.3.4** 直流偏移及其对积分器的影响。

#### 2.3.1 Integrator 积分器

> ◎ 核心：**反相积分器**——$v_+$ 接地；$v_i$ 经 $R$ 到 $v_-$；$C$ 从 $v_o$ 负反馈到 $v_-$。负反馈 + 有限 $A_d$ 很大 → **虚地** $v_-\approx 0$；$v_-$ 处 KCL 得 $\dfrac{dv_o}{dt}=-\dfrac{v_i}{RC}$，即 $\boxed{v_o=-\dfrac{1}{RC}\displaystyle\int v_i\,dt + K}$。
...
| 阶跃宽度 $T_{\mathrm{step}}$ | $|v_o|$ 峰值（线性区） | 说明 |
|------------------------------|----------------------|------|
