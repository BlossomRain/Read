...
右栏 **model** 与左栏电路一一对应（版式对齐 [2.8.2](#282-modeling-for-current-voltage-converter-电流电压转换器模型) I–V 双面板；**$R_i$ 顶与 $G v_i$ 顶断开**）：

| 左：v–i converter | 右：model |
|-------------------|-----------|
| **$v_i$** 接 **$v_+$** | **输入回路**：$v_i$ ∥ **$R_i$** 对地（顶部分离） |
| 反馈 **$R$**（顶母线 **$v_-\to v_o$**，与 I–V 对偶） | **输出回路**：**$G v_i$** 电流源 → $i_o$；**$R_o$** ∥ **$R_L$** 对地；共地 |
| **虚短** $v_-\approx v_i$；**$i_o$** 自运放输出端子引出 | 主关系 **$i_o=G v_i$** |

| 模型元件 | 物理意义 | 理想结论 |
|----------|----------|----------|
| **$v_i$** | 同相输入 | **虚短** $v_-\approx v_i$ |
| **$R$**（$v_-\to v_o$ 负反馈） | 闭环跨导设定 | KCL → **$G=1/R$**，$i_o=v_i/R$ |
| **$R_i$** | 向 **$v_+$** 看入的电阻 | $\boxed{R_i \approx \infty}$（$i_+\approx 0$） |
| **$G v_i$** | 跨导受控源 $i_o=G v_i$ | $\boxed{G = 1/R}$，$i_o=v_i/R$ |
| **$R_o$** | 输出 Norton 电阻 | $\boxed{R_o \approx \infty}$（理想电流源） |
| **$R_L$** | 负载（端口 2） | 由外接电路决定；过小易饱和 |
...
> ◎ **I–I 放大器**：**电流进、电流出**；输入侧与 [2.7.2](#272-current-voltage-conversion-电流电压转换) I–V 同为 **虚地** + **$v_+$** 接地，多一支路 **$R_2$** 串在 **$i_i$** 与 **$v_-$** 之间。理想 **$\boxed{R_i\approx 0}$，$\boxed{\alpha=-R_2/R_1}$，$\boxed{R_o\approx\infty}$**。

##### ∎ 电路与模型（左右对照）

![I-I converter and port equivalent model](./image/电子学一/figures/02-op-ii-converter-model-diagram-01.svg)

| 左：i–i converter | 右：model |
|-------------------|-----------|
| **$i_i$** 经 **$R_2$** 进 **$v_-$**；**$v_+$** 接地（版式对齐 [2.8.3](#283-modeling-for-voltage-current-converter-电压电流转换器模型) V–I 左栏） | **输入回路**：$i_i$ → **$R_i$** 对地（顶部分离） |
| 顶母线 **反馈线** $v_o\to v_-$（**不画** $R_1$，见下表）；**$i_o$** 自输出端引出 | **输出回路**：**$\alpha i_i$** 电流源 → $i_o$；**$R_o$** ∥ **$R_L$** 对地 |
| **虚地** $v_-\approx 0$；**$R_1$** 定 **$\alpha$** | 主关系 **$i_o=\alpha i_i$** |

| 模型元件 | 物理意义 | 理想结论 |
|----------|----------|----------|
| **$R_2$** | 输入电流在 **$v_-$** 上建立 **$i_i R_2$** 压降 | 与 **$R_1$** 上压降相等（虚地钳位） |
| **$R_1$** | 反馈定输出电流 | **$\alpha=-R_2/R_1$**，$i_o=-(R_2/R_1)i_i$ |
| **$R_i$** | 向 **$v_-$** 看入 | $\boxed{R_i \approx 0}$ |
| **$\alpha i_i$** | 电流增益受控源 | $\boxed{\alpha = -R_2/R_1}$ |
| **$R_o$** | 输出 Norton 电阻 | $\boxed{R_o \approx \infty}$ |

**Model it by** 收成 **电流增益** $i_o = \alpha\, i_i$（$\alpha$ 无量纲）。

##### ∎ 理想结论（虚地 + KCL）

$v_+\approx 0$ → **虚地** $v_-\approx 0$。**$R_2$** 上 **$v_{R_2}=i_i R_2$**，反馈迫使 **$R_1$** 上同压 → 输出端电流 **$i_o=-v_{R_1}/R_1$**：

$$
\boxed{i_o = -\frac{R_2}{R_1}\, i_i \;\Rightarrow\; \alpha = -\frac{R_2}{R_1}}.
$$

**$R_i\approx 0$**、**$R_o\approx\infty$** 与 I–V 输入、V–I 输出同类；对照四块：**I–V** $A=-R$，**V–I** $G=1/R$，**I–I** $\alpha=-R_2/R_1$。

##### ∎ 与 h 参数（[2.8.1](#281-amplifier-ac-modeling-放大器交流模型)）

I–I 块 **输入 $i$、输出 $i$**；理想单向 → **$i_o = h_{21} i_i$**，**$h_{21}=\alpha=-R_2/R_1$**。

##### ⚠ 易错点

- **$\alpha$** 是 **电流比**，不是跨阻/跨导；**$R_1$** 为反馈、**$R_2$** 为输入，勿与 I–V 单只 **$R$** 混记。
- **$R_i\approx 0$**、**$R_o\approx\infty$** 与 I–V **输入**、V–I **输出** 同侧——四块端口表宜对照记忆。

##### ✓ 小结

- 理想模型：**$R_i\approx 0$**，**$\alpha=-R_2/R_1$**，**$R_o\approx\infty$**；$i_o=-(R_2/R_1)i_i$。
- 左图 **i–i converter** ↔ 右图 **$R_i$–$\alpha i_i$–$R_o$–$R_L$** 端口等效。
